...
- Create issues via GitHub REST API (idempotent, creates missing `role:*` labels):
  ...
  If you want to avoid re-pasting tokens across sessions (Windows):
  - Recommended: `gh auth login` + `powershell -NoProfile -File scripts/create-agent-issues.ps1 -Mode gh`
  - Alternative: `powershell -NoProfile -File scripts/setup-github-token.ps1` once, then:
    - `powershell -NoProfile -File scripts/run-create-agent-issues-rest-saved.ps1`
  - Details: [docs/runbooks/GITHUB_AUTH_PERSISTENT.md](../runbooks/GITHUB_AUTH_PERSISTENT.md)
