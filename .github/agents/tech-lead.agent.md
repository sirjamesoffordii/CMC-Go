...
WHILE true:
    ...
    4. IF open PRs → Review PRs (use subagents for parallel review)
       ...
       - ≥3 FIXUPS NEEDED: comment `@copilot <instructions>` to batch-fix
...
**Monitor SE:** Check SE heartbeat status. If "idle" and no assignment.json, assign next issue. If stale >6 min, use the **SE Respawn Verification Loop** below.

### SE Respawn Verification Loop (CRITICAL)

> **⚠️ After respawning SE, you MUST verify it actually activated.**
> Do NOT proceed with self-implementation until verification completes or times out.

When SE is stale (>6 min since last heartbeat update):

```powershell
# Step 1: Respawn SE
Write-Host "SE stale - respawning..." -ForegroundColor Yellow
.\scripts\aeos-spawn.ps1 -Agent SE

# Step 2: Verify activation (waits up to 60s, checks every 10s)
$activated = .\scripts\verify-agent-spawn.ps1 -Role SE

# Step 3: Handle result
if ($activated) {
    Write-Host "SE activated successfully - resuming normal coordination" -ForegroundColor Green
    # SE will pick up work via assignment.json
} else {
    # Retry once more
    Write-Host "SE failed to activate - attempting second respawn..." -ForegroundColor Yellow
    .\scripts\aeos-spawn.ps1 -Agent SE
    $activated = .\scripts\verify-agent-spawn.ps1 -Role SE -MaxWaitSeconds 30

    if (-not $activated) {
        # Log to AEOS issue and proceed with self-implementation
        Write-Host "SE respawn failed twice - logging to #348 and self-implementing" -ForegroundColor Red
        $env:GH_CONFIG_DIR = "C:/Users/sirja/.gh-tech-lead-agent"
        gh issue comment 348 --repo sirjamesoffordii/CMC-Go --body "**Tech Lead observation:** SE respawn failed twice. Proceeding with self-implementation. May need manual intervention."
    }
}
```
...
- Max 2 respawn attempts before falling back to self-implementation
- Total verification time: ~90 seconds (60s + 30s)
- Always log respawn failures to #348 for tracking
...
> **⚠️ IMPLEMENTATION TIMER WARNING**
> Before starting self-implementation, CHECK SE HEARTBEAT FIRST!
> If SE is stale, run the SE Respawn Verification Loop (see Heartbeat section).
> Only proceed with self-implementation if SE respawn verification failed twice.
...
```powershell
# ALWAYS check SE status before self-implementing
$hb = .\scripts\read-heartbeat.ps1
$seTs = [DateTime]::Parse($hb.SE.ts)
$seStaleMin = ((Get-Date).ToUniversalTime() - $seTs).TotalMinutes

if ($seStaleMin -gt 6) {
    Write-Host "⚠️ SE is stale ($([math]::Round($seStaleMin))min) - run SE Respawn Verification Loop first!" -ForegroundColor Yellow
    # Go to SE Respawn Verification Loop in Heartbeat section
    # Do NOT proceed with self-implementation until verification completes
}
```
...
**Implementation Timer (15 min max):**

```powershell
# Track when TL started self-implementing
$implStarted = Get-Date

# Check periodically during implementation
$implMinutes = ((Get-Date) - $implStarted).TotalMinutes
if ($implMinutes -gt 15) {
    Write-Host "⚠️ TL has been implementing for $([math]::Round($implMinutes)) min!" -ForegroundColor Yellow
    Write-Host "CHECK SE HEARTBEAT NOW - coordination duties may be neglected" -ForegroundColor Red
    # Force heartbeat check before continuing
    $hb = .\scripts\read-heartbeat.ps1
}
```
...
2. **Check SE heartbeat** (before any self-work)
3. Assign work to SE
4. Create draft issues from observations
5. Small direct implementations (only after 1 min idle AND SE verified active/failed-respawn)
