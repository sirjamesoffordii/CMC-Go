# Runbooks Index

Single entrypoint for operational playbooks, setup/tooling references, and agent runbooks.

## Setup & workflow

- Dev setup: [DEV_SETUP.md](DEV_SETUP.md)
- Local setup (shared demo DB): [LOCAL_SETUP.md](LOCAL_SETUP.md)
- Shared demo DB details: [SHARED_DEMO_DB.md](SHARED_DEMO_DB.md)

## Database setup & safety

- Verify schema + writes: [DB_SETUP_VERIFICATION.md](DB_SETUP_VERIFICATION.md)
- Database tooling summary: [DATABASE_TOOLING_SUMMARY.md](DATABASE_TOOLING_SUMMARY.md)
- Reset DB (dev): [DB_RESET_GUIDE.md](DB_RESET_GUIDE.md)
- Local DB sync notes: [LOCAL_DB_SYNC.md](LOCAL_DB_SYNC.md)
- DB health guardrails: [DB_HEALTH_GUARDRAILS.md](DB_HEALTH_GUARDRAILS.md)

## Migrations & seeding

- Migration guide: [MIGRATION_GUIDE.md](MIGRATION_GUIDE.md)
- Migration rules: [MIGRATION_RULES.md](MIGRATION_RULES.md)
- Seed guide: [SEED_GUIDE.md](SEED_GUIDE.md)

## Observability & release hygiene

- Sentry error monitoring: [SENTRY_ERROR_MONITORING.md](SENTRY_ERROR_MONITORING.md)
- Coverage + AI review notes: [CODE_COVERAGE_AND_AI_CODE_REVIEW.md](CODE_COVERAGE_AND_AI_CODE_REVIEW.md)

## Coherence

- Coherence Engine: [COHERENCE_ENGINE.md](COHERENCE_ENGINE.md)

## Ops log

- Ops change log: [ops-change-log.md](ops-change-log.md)

## Agent runbooks

Agent-specific runbooks:

- Evidence standard: [AGENT_EVIDENCE_STANDARD.md](AGENT_EVIDENCE_STANDARD.md)
- Quick commands: [AGENT_QUICK_COMMANDS.md](AGENT_QUICK_COMMANDS.md)
- GitHub auth notes: [GITHUB_AUTH.md](GITHUB_AUTH.md)
