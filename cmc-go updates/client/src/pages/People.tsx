...
export default function People() {
  ...
  // Track per‑campus sorting preference. Each campusId maps to the selected sort key.
  // The default sort (when undefined) is by "status".
  ...
  const districtsWithData = useMemo(() => {
    ...
    districtMap.forEach((data) => {
      // Sort campuses alphabetically by name first
      ...
      sortedCampuses.forEach(item => {
        // Determine the sort key for this campus; default to status
        const sortKey = campusSorts[item.campus.id] ?? 'status';
        // Copy the people array so we don't mutate the original
        const peopleCopy = [...item.people];
        if (sortKey === 'status') {
          // Define an order for statuses. Unknown statuses default to the end.
          ...
          peopleCopy.sort((pA, pB) => {
            const orderA = statusOrder[pA.status] ?? 99;
            const orderB = statusOrder[pB.status] ?? 99;
            if (orderA !== orderB) return orderA - orderB;
            // fallback to name if statuses are equal
            return (pA.name ?? '').localeCompare(pB.name ?? '');
          });
        } else if (sortKey === 'name') {
          peopleCopy.sort((pA, pB) => (pA.name ?? '').localeCompare(pB.name ?? ''));
        } else if (sortKey === 'role') {
          peopleCopy.sort((pA, pB) => (pA.primaryRole ?? '').localeCompare(pB.primaryRole ?? ''));
        }
        newMap.set(item.campus.id, { campus: item.campus, people: peopleCopy });
      });
  ...
  }, [filteredPeople, allDistricts, allCampuses, campusSorts]);
  ...
  const handleCampusSortChange = (campusId: number, sortBy: 'status' | 'name' | 'role') => {
    // Update the selected sort for this campus. This will trigger a re‑render via campusSorts dependency in useMemo.
