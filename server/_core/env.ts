  ...
  if (process.env.DATABASE_URL) {
    ...
    if (!isProduction) {
      try {
        ...
        if (error instanceof Error && error.message.includes('Root is blocked')) {
          throw error;
        }
...
export function getDatabaseInfo() {
  if (!databaseUrl) {
    return { host: null, database: null, isDemoDb: false, connectionPath: null };
  }
...
if (!process.env.NODE_ENV || process.env.NODE_ENV !== "production") {
  if (databaseUrl) {
    ...
    if (dbInfo.host && dbInfo.database) {
      ...
      if (!dbInfo.host.includes('proxy.rlwy.net') && !dbInfo.host.includes('railway.internal')) {
      ...
      if (dbInfo.isDemoDb) {
        console.log(`[ENV] ⚠️  Connected to Railway staging demo DB (shared with staging website)`);
      }
    } else {
      const urlObj = new URL(databaseUrl);
