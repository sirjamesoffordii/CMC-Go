...
const EXCEL_PATH =
  "/home/ubuntu/upload/CMC_Go_Seed_REAL_Campuses_1105_UniquePeople_Assignments_FINAL.xlsx";
...
async function main() {
  ...
  if (fileSizeInMB > 100) {
    console.error(
      `ERROR: Excel file too large (${fileSizeInMB.toFixed(2)}MB). Maximum allowed is 100MB.`
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(
      "ERROR: Failed to parse Excel file. File may be corrupted or malformed."
    );
  ...
  for (const person of people) {
    ...
    const primaryCampusId = primaryCampusName
      ? campusNameToId.get(primaryCampusName)
      : null;
  ...
  // ============ SUMMARY ============
  console.log("\n=== Summary ===");
  const [[{ count: campusCount }]] = await conn.execute(
    "SELECT COUNT(*) as count FROM campuses"
  );
  console.log(`Total campuses in DB: ${campusCount}`);

  const [[{ count: totalPeople }]] = await conn.execute(
    "SELECT COUNT(*) as count FROM people"
  );
  console.log(`Total people in DB: ${totalPeople}`);

  const [[{ count: totalAssignments }]] = await conn.execute(
