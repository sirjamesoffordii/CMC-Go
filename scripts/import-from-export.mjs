...
const exportData = JSON.parse(
  readFileSync(join(__dirname, "../data/export.json"), "utf-8")
);
  ...
  if (exportData.districts && exportData.districts.length > 0) {
    ...
    for (const district of exportData.districts) {
      try {
        ...
        console.warn(
          `Failed to import district ${district.id}:`,
          error.message
        );
      }
    }
  ...
  if (exportData.people && exportData.people.length > 0) {
    ...
    for (const person of exportData.people) {
      try {
        await db
          ...
          .values({
            ...
            statusLastUpdated: person.statusLastUpdated
              ? new Date(person.statusLastUpdated * 1000)
              : null,
            ...
            lastEdited: person.lastEdited
              ? new Date(person.lastEdited * 1000)
              : null,
            lastEditedBy: person.lastEditedBy || null,
            createdAt: person.createdAt
              ? new Date(person.createdAt * 1000)
              : new Date(),
          })
          .onDuplicateKeyUpdate({
            set: {
        ...
        console.warn(
          `Failed to import person ${person.name} (${person.personId}):`,
          error.message
        );
        errorCount++;
      }
  ...
  if (exportData.assignments && exportData.assignments.length > 0) {
    ...
    for (const assignment of exportData.assignments) {
      try {
        await db
          ...
          .values({
            ...
            createdAt: assignment.createdAt
              ? new Date(assignment.createdAt * 1000)
              : new Date(),
          })
          .onDuplicateKeyUpdate({
            set: {
        ...
        console.warn(
          `Failed to import assignment ${assignment.id}:`,
          error.message
        );
        errorCount++;
      }
    ...
    console.log(
      `✅ Imported ${successCount} assignments (${errorCount} errors)\n`
    );
  }
        ...
        await db
          ...
          .values({
            ...
            createdAt: need.createdAt
              ? new Date(need.createdAt * 1000)
              : new Date(),
          })
          .onDuplicateKeyUpdate({
            set: {
              description:
                need.description || need.notes || need.type || "Need",
              amount: need.amount || null,
            },
          });
        successCount++;
        ...
        await db
          ...
          .values({
            ...
            updatedAt: setting.updatedAt
              ? new Date(setting.updatedAt * 1000)
              : new Date(),
          })
          .onDuplicateKeyUpdate({
            set: {
