...
function extractColumnsFromMigration(sqlContent, tableName) {
  ...
  const createTableMatch = sqlContent.match(
    new RegExp(`CREATE TABLE[\\s\\S]*?\\\`?${tableName}\\\`?[\\s\\S]*?\\(([\\s\\S]*?)\\)`, "i")
  );
...
async function auditMigrations() {
  ...
  try {
    ...
    try {
      ...
      console.warn("⚠️  Migrations directory not found or empty:", migrationsDir);
      console.warn("   Run 'pnpm db:generate' to create migrations\n");
    }
    
    if (migrationFiles.length === 0) {
      console.log("⚠️  No migration files found");
      console.log("   This is expected if migrations haven't been generated yet.");
      console.log("   Run 'pnpm db:generate' to create migrations from schema.ts\n");
      return;
    }
    ...
    Array.from(migrationTables).sort().forEach((t) => console.log(`  - ${t}`));
    console.log();
    ...
    if (tablesNotInMigrations.length > 0) {
      ...
      console.log();
      console.log("   These tables may have been created manually or via db:push.");
    ...
    for (const table of Array.from(migrationTables)) {
      if (actualTables.includes(table)) {
        try {
          ...
          if (migrationCols.size > 0 && actualColumns.size > migrationCols.size) {
            const missing = Array.from(actualColumns).filter((c) => !migrationCols.has(c));
            if (missing.length > 0) {
              console.log(`  ⚠️  ${table}: ${missing.length} column(s) not in migrations: ${missing.join(", ")}`);
