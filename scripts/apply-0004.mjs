...
async function apply() {
  const filePath = join(process.cwd(), 'drizzle', '0004_add_people_household_fields.sql');
  ...
  try {
    for (const stmt of statements) {
      try {
        ...
        if (msg.includes('Duplicate column name') || msg.includes('already exists') || err.code === 'ER_DUP_FIELDNAME') {
