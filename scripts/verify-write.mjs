...
import { campuses, people } from "../drizzle/schema.ts";
...
async function verifyWrites() {
  ...
  try {
    ...
    if (readCampus.length === 0 || readCampus[0].name !== testCampus.name) {
      throw new Error(
        `Campus write failed: expected ${testCampus.name}, got ${readCampus[0]?.name || "nothing"}`
      );
    }

    console.log(
      `   ✅ Campus created and read back: ${readCampus[0].name} (id: ${campusId})`
    );
    ...
    if (
      readPerson.length === 0 ||
      readPerson[0].personId !== testPerson.personId
    ) {
      throw new Error(
        `Person write failed: expected ${testPerson.personId}, got ${readPerson[0]?.personId || "nothing"}`
      );
    }

    console.log(
      `   ✅ Person created and read back: ${readPerson[0].name} (personId: ${readPerson[0].personId})`
    );
    ...
    if (
      updatedPerson.length === 0 ||
      updatedPerson[0].status !== updatedStatus
    ) {
      throw new Error(
        `Person update failed: expected status ${updatedStatus}, got ${updatedPerson[0]?.status || "nothing"}`
      );
    }

    console.log(
      `   ✅ Person updated and read back: status = ${updatedPerson[0].status}`
    );
