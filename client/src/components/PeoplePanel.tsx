...
export function PeoplePanel({ onClose }: PeoplePanelProps) {
  ...
  const [needFilter, setNeedFilter] = useState<Set<"Financial" | "Transportation" | "Housing" | "Other" | "Need Met">>(new Set());
  const [roleFilter, setRoleFilter] = useState<Set<string>>(new Set());
  const [familyGuestFilter, setFamilyGuestFilter] = useState<Set<"spouse" | "children" | "guest">>(new Set());
  const [depositPaidFilter, setDepositPaidFilter] = useState<boolean | null>(null); // null = no filter, true = paid, false = not paid
  const [searchQuery, setSearchQuery] = useState("");
  const [myCampusOnly, setMyCampusOnly] = useState(false);
  const [sortBy, setSortBy] = useState<"Region" | "District" | "Campus" | "Individual" | "Update">("District");
  const [order, setOrder] = useState<"Greatest first" | "Least first">("Greatest first");
  ...
  const filteredPeople = useMemo(() => {
    ...
    if (needFilter.size > 0) {
      filtered = filtered.filter((p: any) => {
        ...
        const selectedNeedTypes = Array.from(needFilter).filter(need => need !== "Need Met");
        if (selectedNeedTypes.length > 0) {
          const hasSelectedNeedType = selectedNeedTypes.some(needType => personNeedTypes.has(needType));
    ...
    return filtered;
  }, [allPeople, allNeeds, statusFilter, needFilter, roleFilter, familyGuestFilter, depositPaidFilter, searchQuery, myCampusOnly, user?.campusId]);
  ...
  const groupedData = useMemo(() => {
    if (sortBy === "Individual") {
      ...
      const sorted = [...filteredPeople].sort((a, b) => {
        ...
        return order === "Greatest first" ? nameB.localeCompare(nameA) : nameA.localeCompare(nameB);
      });
    ...
    if (sortBy === "Update") {
      ...
      const sorted = [...filteredPeople].sort((a, b) => {
        const dateA = a.statusLastUpdated ? new Date(a.statusLastUpdated).getTime() : 0;
        const dateB = b.statusLastUpdated ? new Date(b.statusLastUpdated).getTime() : 0;
    ...
    if (sortBy === "District") {
      ...
      const districts = Array.from(districtMap.values()).map(data => {
        ...
        const countA = Array.from(a.campuses.values()).reduce((sum, c) => sum + c.people.length, 0) + a.unassigned.length;
        const countB = Array.from(b.campuses.values()).reduce((sum, c) => sum + c.people.length, 0) + b.unassigned.length;
        if (countA !== countB) {
          return order === "Greatest first" ? countB - countA : countA - countB;
        }
    ...
    allDistricts.forEach(district => {
      const region = district.region || DISTRICT_REGION_MAP[district.id] || "Unknown";
    ...
    const regions = Array.from(regionMap.values()).map(regionData => {
      const districts = Array.from(regionData.districts.values()).map(districtData => {
        const sortedCampuses = Array.from(districtData.campuses.values()).sort((a, b) => {
          const countA = a.people.length;
          const countB = b.people.length;
          if (countA !== countB) {
            return order === "Greatest first" ? countB - countA : countA - countB;
      ...
      const countA = a.districts.reduce((sum, d) => 
        sum + Array.from(d.campuses.values()).reduce((s, c) => s + c.people.length, 0) + d.unassigned.length, 0
      );
      const countB = b.districts.reduce((sum, d) => 
        sum + Array.from(d.campuses.values()).reduce((s, c) => s + c.people.length, 0) + d.unassigned.length, 0
      );
      if (countA !== countB) {
        return order === "Greatest first" ? countB - countA : countA - countB;
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Popover>
              ...
              <PopoverContent className="w-56 p-2" align="start">
                <div className="space-y-1">
                  {(["Financial", "Transportation", "Housing", "Other", "Need Met"] as const).map((need) => {
              ...
              <PopoverContent className="w-56 p-2 max-h-64 overflow-y-auto" align="start">
                <div className="space-y-1">
                  {uniqueRoles.map((role) => {
                    const count = allPeople.filter((p: any) => p.primaryRole === role).length;
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm text-gray-600 font-medium">Family & Guest:</span>
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                ...
                >
                  <span>
                    {familyGuestFilter.size === 0
                      ...
                      ? Array.from(familyGuestFilter)[0] === 'spouse' ? 'Spouse' : 
                        Array.from(familyGuestFilter)[0] === 'children' ? 'Children' : 'Guest'
        ...
        {(() => {
          ...
          if (groupedData.type === "individual") {
            if (groupedData.people.length === 0) {
              return <div className="p-6 text-center text-gray-500">No people found</div>;
            }
            return (
              <div className="divide-y divide-gray-200">
                {groupedData.people.map((person: any) => renderPerson(person, "px-4"))}
              </div>
            );
          }
          ...
          if (groupedData.type === "campus") {
            if (groupedData.campuses.length === 0) {
              return <div className="p-6 text-center text-gray-500">No campuses found</div>;
            }
            return (
              <div className="divide-y divide-gray-200">
                {groupedData.campuses.map(({ campus, people }) => {
                  ...
                  return (
                    <div key={campus.id} className="divide-y divide-gray-100">
                      <div className="px-4 py-3 hover:bg-gray-50 cursor-pointer transition-colors" onClick={() => toggleCampusExpansion(campus.id)}>
                      ...
                      {isCampusExpanded && people.length > 0 && (
                        <div className="bg-gray-50">
                          {people.map((person: any) => renderPerson(person, "px-8"))}
                        </div>
                      )}
                    </div>
                  );
          ...
          if (groupedData.type === "district") {
            if (groupedData.districts.length === 0) {
              return <div className="p-6 text-center text-gray-500">No districts found</div>;
            }
            return (
              <div className="divide-y divide-gray-200">
                {groupedData.districts.map(({ district, campuses, unassigned }) => {
                  ...
                  return (
                    <div key={district.id} className="divide-y divide-gray-100">
                      <div className="px-4 py-3 hover:bg-gray-50 cursor-pointer transition-colors" onClick={() => toggleDistrictExpansion(district.id)}>
                        <div className="flex items-center gap-3">
                          {isDistrictExpanded ? (
                            <ChevronDown className="w-4 h-4 text-gray-400 flex-shrink-0" />
                          ) : (
                            ...
                            return (
                              <div key={campus.id} className="divide-y divide-gray-100">
                                <div className="px-8 py-2 hover:bg-gray-100 cursor-pointer transition-colors">
                                  <button onClick={(e) => { e.stopPropagation(); toggleCampusExpansion(campus.id); }} className="flex items-center gap-2 flex-1 w-full text-left">
                                    {isCampusExpanded ? (
                                ...
                                {isCampusExpanded && people.length > 0 && (
                                  <div className="bg-white">
                                    {people.map((person: any) => renderPerson(person, "px-12"))}
                                  </div>
                                )}
                              </div>
                            );
                          ...
                          {unassigned.length > 0 && (
                            <div className="px-8 py-2">
                              ...
                              <div className="bg-white rounded border border-gray-200">
                                {unassigned.map((person: any) => renderPerson(person, "px-4"))}
                              </div>
                            </div>
                          )}
                        </div>
          ...
          if (groupedData.regions.length === 0) {
            return <div className="p-6 text-center text-gray-500">No regions found</div>;
          }
          return (
            <div className="divide-y divide-gray-200">
              {groupedData.regions.map(({ region, districts }) => {
                const isRegionExpanded = expandedRegions.has(region);
                const totalPeople = districts.reduce((sum, d) => 
                  sum + Array.from(d.campuses.values()).reduce((s, c) => s + c.people.length, 0) + d.unassigned.length, 0
                );
                return (
                  <div key={region} className="divide-y divide-gray-100">
                    <div className="px-4 py-3 hover:bg-gray-50 cursor-pointer transition-colors" onClick={() => toggleRegionExpansion(region)}>
                      <div className="flex items-center gap-3">
                        {isRegionExpanded ? (
                          <ChevronDown className="w-4 h-4 text-gray-400 flex-shrink-0" />
                        ) : (
                    ...
                    {isRegionExpanded && (
                      <div className="bg-gray-50">
                        {districts.map(({ district, campuses, unassigned }) => {
                          ...
                          return (
                            <div key={district.id} className="divide-y divide-gray-100">
                              <div className="px-8 py-2 hover:bg-gray-100 cursor-pointer transition-colors" onClick={(e) => { e.stopPropagation(); toggleDistrictExpansion(district.id); }}>
                                <div className="flex items-center gap-2">
                                  {isDistrictExpanded ? (
                                    <ChevronDown className="w-3 h-3 text-gray-400 flex-shrink-0" />
                                  ) : (
                                  ...
                                  <div className="flex-1">
                                    <h4 className="font-medium text-gray-900 text-sm">{district.name}</h4>
                                  </div>
                                  ...
                                  {Array.from(campuses.values()).map(({ campus, people }) => {
                                    ...
                                    return (
                                      <div key={campus.id} className="divide-y divide-gray-100">
                                        ...
                                        {isCampusExpanded && people.length > 0 && (
                                          <div className="bg-gray-50">
                                            {people.map((person: any) => renderPerson(person, "px-16"))}
                                          </div>
                                        )}
                                      </div>
                                    );
