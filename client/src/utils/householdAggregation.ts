 ...
 */
export function calculateHouseholdTotals(
  people: Person[],
  ...
  people.forEach(person => {
    if (person.householdId) {
      ...
      const childrenCount =
        ...
        (person.kids ? parseInt(person.kids, 10) || 0 : 0);
      const guestsCount =
        person.guestsCount ??
        (person.guests ? parseInt(person.guests, 10) || 0 : 0);
  ...
  // Sum solo people
  const soloChildrenTotal = Array.from(soloChildren.values()).reduce(
    (sum, count) => sum + count,
    0
  );
  const soloGuestsTotal = Array.from(soloGuests.values()).reduce(
    (sum, count) => sum + count,
    0
  );
  ...
  const families =
    countedHouseholdIds.size +
    (soloChildren.size > 0 || soloGuests.size > 0 ? 1 : 0);
